...
uint8_t touchpad_taskID = INVALID_TASK_ID;

static pct1336_params_t params[] = {
    {.params.bank = 0, .params.address = 0x12, .params.data = 0x42},
    {.params.bank = 0, .params.address = 0x13, .params.data = 0x00},
    {.params.bank = 0, .params.address = 0x14, .params.data = 0x42},
...
static uint32_t osal_ms_to_system_time(uint32_t time_ms)
{
    ...
    system_time = ((uint64_t)time_ms * 1000ULL + (uint64_t)SYSTEM_TIME_MICROSEN - 1ULL) /
                  (uint64_t)SYSTEM_TIME_MICROSEN;
    if (system_time > UINT32_MAX) {
        return UINT32_MAX;
    }
...
static uint32_t osal_system_time_to_ms(uint32_t system_time)
{
    uint64_t time_ms;
    ...
    time_ms = ((uint64_t)system_time * (uint64_t)SYSTEM_TIME_MICROSEN + 999ULL) / 1000ULL;
    if (time_ms == 0U) {
        return 1U;
    }
...
{
    if (touchpad_taskID != INVALID_TASK_ID) {
        return;
    }
...
static uint16_t touchpad_process_event(uint8_t task_id, uint16_t events)
{
    ...
    if (events & REPORT_SEND_EVT) {
        if (report_buffer_task()){
            ...
            tmos_start_task(commu_taskID, REPORT_DELETE_PAK_EVT, osal_ms_to_system_time(REPORT_RESCHEDULE_DELAY_MS));
        }
        /* 有在等待，可以重新发送的报告就继续调度发送事件 */
        if (report_buffer_waiting_send()) {
            tmos_start_task(commu_taskID, REPORT_SEND_EVT, osal_ms_to_system_time(REPORT_RESCHEDULE_DELAY_MS));
        }
